...
function idInvalidCardCompanies(invalidCards) {
  ...
  for (let i=0; i<invalidCards; i++) {
    switch(invalidCards[i][0]) {
      case 3:
        if (credCompanies.indexOf('Amex') === -1) {
          credCompanies.push('Amex')
        }
      
      case 4:
        if (credCompanies.indexOf('Visa') === -1) {
          credCompanies.push('Visa')
        }

      case 5: 
        if (credCompanies.indexOf('Mastercard') === -1) {
          credCompanies.push('Mastercard')
        }

      case 6: 
        if (credCompanies.indexOf('Discover') === -1) {
